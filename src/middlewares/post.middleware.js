...
const existsModelByIdImg = (modelo) => {
  return async (req, res, next) => {
    ...
    if (!data) {
      return res
        .status(404)
        .json({ message: `El id de la imagen ${id} no se encuentra registrado` });
    }
    next();
  };
};

const validaIdImg = (req, res, next) => {
  const id = req.params.imgId;
  if (id <= 0) {
    return res
      .status(400)
      .json({ message: "Bad Request: No pueden ser un id negativo de imagen" });
  }
  next();
};
...
module.exports = { existsModelById, validaId, existsModelByIdImg,validaIdImg,schemaValidator};
